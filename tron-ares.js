// --------- DATA MODEL ---------
const channels = [];   // { id, name, url, logo, group, isIframe, isFavorite }
const iframeItems = []; // overlays
...
function createChannelElement(entry, index, sourceType) {
  ...
  if (!entry.isIframe && index === currentIndex && sourceType === 'channel') {
    li.classList.add('active');
  }
  if (entry.isIframe && index === currentIframeIndex && sourceType === 'iframe') {
    li.classList.add('active');
  }
...
function renderChannelList() {
  ...
  channels.forEach((ch, idx) => {
    const el = createChannelElement(ch, idx, 'channel');
...
function renderFavoritesList() {
  ...
  const favs = [
    ...channels.filter(c => c.isFavorite && !c.isIframe),
    ...iframeItems.filter(i => i.isFavorite && i.isIframe)
  ];
  favs.forEach((entry, idx) => {
    const el = createChannelElement(entry, idx, entry.isIframe ? 'iframe' : 'channel');
...
function playUrl(entry) {
  ...
  // 🔁 RÈGLE SPÉCIALE :
  // si l'URL contient rtp.pt ou "smil:" → on force directement le lecteur externe
  const forceExternal =
    /rtp\.pt/i.test(url) ||   // flux RTP (comme streaming-live.rtp.pt)
    /smil:/i.test(url);       // manifestes SMIL (smil:rtp1HD.smil, etc.)
  ...
  // IFRAME / YouTube classiques
  if (entry.isIframe || isYoutubeUrl(url)) {
    playEntryAsOverlay(entry);
    return;
  }

  // Lecture normale dans le player vidéo
  showVideo();
  destroyHls();
  destroyDash();
...
function parseM3U(content) {
  ...
  for (const rawLine of lines) {
    ...
    if (line.startsWith('#EXTINF')) {
      ...
      const entry = {
        id: 'ch-' + (channels.length + results.length + 1),
        name,
        url,
